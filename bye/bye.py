...
default_greeting = "bye {0.name} to {1.name}!"
...
class Bye:
    ...
    async def byeset(self, ctx):
        """Sets bye module settings"""
        ...
        if ctx.invoked_subcommand is None:
            ...
            msg += "CHANNEL: #{}\n".format(self.get_bye_channel(server))
    ...
    @byeset.group(pass_context=True, name="msg")
    async def byeset_msg(self, ctx):
        """Manage bye messages
        """
    ...
    async def byeset_msg_add(self, ctx, *, format_msg):
        ...
        fileIO(settings_path, "save", self.settings)
        await self.bot.say("bye message added for the server.")
        await self.send_testing_msg(ctx, msg=format_msg)

    @byeset_msg.command(pass_context=True, name="del", no_pm=True)
    async def byeset_msg_del(self, ctx):
        """Removes a bye message from the random message list
        """
        server = ctx.message.server
        author = ctx.message.author
        msg = 'Choose a bye message to delete:\n\n'
    ...
    @byeset_msg.command(pass_context=True, name="list", no_pm=True)
    async def byeset_msg_list(self, ctx):
        """Lists the bye messages of this server
        """
        server = ctx.message.server
        msg = 'bye messages:\n\n'
        for c, m in enumerate(self.settings[server.id]["GREETING"]):
    ...
    async def toggle(self, ctx):
        ...
        if self.settings[server.id]["ON"]:
            await self.bot.say("I will now bye new users to the server.")
            await self.send_testing_msg(ctx)
        else:
            await self.bot.say("I will no longer bye new users.")
        fileIO(settings_path, "save", self.settings)
    ...
    async def channel(self, ctx, channel : discord.Channel=None):
        ...
        fileIO(settings_path, "save", self.settings)
        channel = self.get_bye_channel(server)
        await self.bot.send_message(channel, "I will now send bye "
                                    "messages to {0.mention}".format(channel))
        await self.send_testing_msg(ctx)

    @byeset.group(pass_context=True, name="bot", no_pm=True)
    async def byeset_bot(self, ctx):
        """Special bye for bots"""
        if ctx.invoked_subcommand is None or \
                isinstance(ctx.invoked_subcommand, commands.Group):
            await send_cmd_help(ctx)
            return
    ...
    async def byeset_bot_msg(self, ctx, *, format_msg=None):
        ...
        if format_msg is None:
            await self.bot.say("Bot message reset. Bots will now be byed as regular users.")
        else:
            await self.bot.say("Bot bye message set for the server.")
    ...
    async def member_join(self, member):
        ...
        bot_bye = member.bot and self.settings[server.id]["BOTS_MSG"]
        bot_role = member.bot and self.settings[server.id]["BOTS_ROLE"]
        msg = bot_bye or rand_choice(self.settings[server.id]["GREETING"])
        ...
        if not member.bot and self.settings[server.id]["WHISPER"]:
            try:
                ...
                print("bye.py: unable to whisper {}. Probably "
                      "doesn't want to be PM'd".format(member))
        # grab the bye channel
        channel = self.get_bye_channel(server)
        if channel is None:  # complain even if only whisper
            print('bye.py: Channel not found. It was most '
                  'likely deleted. User joined: {}'.format(member.name))
            return
        # we can stop here
        if only_whisper and not bot_bye:
            return
...
def check_folders():
    if not os.path.exists("data/bye"):
        print("Creating data/bye folder...")
        os.makedirs("data/bye")


def check_files():
    f = settings_path
    if not fileIO(f, "check"):
        print("Creating bye settings.json...")
        fileIO(f, "save", {})
